...
bool multi_map_server__msg__multi_occupancy_grid__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("multi_map_server.msg._multi_occupancy_grid.MultiOccupancyGrid", full_classname_dest, 61) == 0);
  }
  multi_map_server__msg__MultiOccupancyGrid * ros_message = _ros_message;
  {  // maps
    ...
    if (!nav_msgs__msg__OccupancyGrid__Sequence__init(&(ros_message->maps), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create nav_msgs__msg__OccupancyGrid__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    nav_msgs__msg__OccupancyGrid * dest = ros_message->maps.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!nav_msgs__msg__occupancy_grid__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
{
  /* NOTE(esteve): Call constructor of MultiOccupancyGrid */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("multi_map_server.msg._multi_occupancy_grid");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "MultiOccupancyGrid");
  ...
  {  // maps
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      PyObject * pyitem = nav_msgs__msg__occupancy_grid__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
