...
#include "multi_map_server/msg/detail/sparse_map3_d__rosidl_typesupport_fastrtps_c.h"
...
#include "multi_map_server/msg/detail/vertical_occupancy_grid_list__functions.h"  // lists
...
using _SparseMap3D__ros_msg_type = multi_map_server__msg__SparseMap3D;
  ...
  eprosima::fastcdr::Cdr & cdr)
{
  ...
  const _SparseMap3D__ros_msg_type * ros_message = static_cast<const _SparseMap3D__ros_msg_type *>(untyped_ros_message);
  // Field name: header
  {
  ...
  // Field name: lists
  {
    const message_type_support_callbacks_t * callbacks =
      static_cast<const message_type_support_callbacks_t *>(
      ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(
        rosidl_typesupport_fastrtps_c, multi_map_server, msg, VerticalOccupancyGridList
      )()->data);
    size_t size = ros_message->lists.size;
    auto array_ptr = ros_message->lists.data;
  ...
  void * untyped_ros_message)
{
  ...
  _SparseMap3D__ros_msg_type * ros_message = static_cast<_SparseMap3D__ros_msg_type *>(untyped_ros_message);
  // Field name: header
  {
    const message_type_support_callbacks_t * callbacks =
      static_cast<const message_type_support_callbacks_t *>(
  ...
  // Field name: lists
  {
    ...
    if (ros_message->lists.data) {
      multi_map_server__msg__VerticalOccupancyGridList__Sequence__fini(&ros_message->lists);
    }
    if (!multi_map_server__msg__VerticalOccupancyGridList__Sequence__init(&ros_message->lists, size)) {
      fprintf(stderr, "failed to create array for field 'lists'");
      return false;
    }
    auto array_ptr = ros_message->lists.data;
    for (size_t i = 0; i < size; ++i) {
  ...
  size_t current_alignment)
{
  ...
  // field.name lists
  {
    ...
    for (size_t index = 0; index < array_size; ++index) {
      current_alignment += get_serialized_size_multi_map_server__msg__VerticalOccupancyGridList(
...
static uint32_t _SparseMap3D__get_serialized_size(const void * untyped_ros_message)
{
  return static_cast<uint32_t>(
    get_serialized_size_multi_map_server__msg__SparseMap3D(
  ...
  size_t current_alignment)
{
  ...
  // member: lists
  {
    ...
    for (size_t index = 0; index < array_size; ++index) {
      ...
      size_t inner_size;
      inner_size =
        max_serialized_size_multi_map_server__msg__VerticalOccupancyGridList(
        inner_full_bounded, inner_is_plain, current_alignment);
      last_member_size += inner_size;
      current_alignment += inner_size;
      full_bounded &= inner_full_bounded;
  ...
  if (is_plain) {
    ...
    using DataType = multi_map_server__msg__SparseMap3D;
    is_plain =
      (
...
static size_t _SparseMap3D__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_multi_map_server__msg__SparseMap3D(
...
static message_type_support_callbacks_t __callbacks_SparseMap3D = {
