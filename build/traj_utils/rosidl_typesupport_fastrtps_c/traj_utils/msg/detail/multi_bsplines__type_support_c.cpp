  ...
  void * untyped_ros_message)
{
  ...
  // Field name: traj
  {
    ...
    if (!traj_utils__msg__Bspline__Sequence__init(&ros_message->traj, size)) {
      fprintf(stderr, "failed to create array for field 'traj'");
      return false;
    }
    auto array_ptr = ros_message->traj.data;
    for (size_t i = 0; i < size; ++i) {
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = traj_utils__msg__MultiBsplines;
    is_plain =
      (
...
static size_t _MultiBsplines__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_traj_utils__msg__MultiBsplines(
